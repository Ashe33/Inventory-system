...
if (isset($_POST['submit_request'])) {
    ...
    if (!$product) {
        $req_error = "Product not found.";
    } elseif ($quantity <= 0) {
        $req_error = "Quantity must be at least 1.";
    } elseif ($quantity > $product['quantity']) {
        $req_error = "Requested quantity exceeds available stock ({$product['quantity']}).";
    } else {
...
<?php if (isset($req_error)): ?>
    <div class="alert alert-error">❌ <?= $req_error ?></div>
<?php endif; ?>
...
<div class="form-card">
    ...
    <form method="POST">
        <div class="form-row">
            ...
            <button type="submit" name="submit_request" class="btn btn-green">
                Submit →
            </button>
        </div>
    </form>
...
<div class="table-card">
    ...
    <table>
        ...
        <?php else: ?>
            <tr class="empty-row">
                <td colspan="6">No requests yet. Submit one above.</td>
            </tr>
        <?php endif; ?>
        </tbody>
    </table>
